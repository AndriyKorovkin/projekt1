...
function aimateCircleY(translateY, direction, condition, onStop) {
    const NewTranslateY = translateY + 1*direction;

    circleEl.style.top = `${NewTranslateY}px`;

    if (!condition(translateY, direction)) {
        delay(1000/25)
            .then(() => aimateCircleY(NewTranslateY, direction, condition, onStop));
    } else {
        onStop();
    }
}
...
function animationPromiseCircleY(translateY, direction, condition) {
    return new Promise(function(resolve) {
        aimateCircleY(translateY, direction, condition, resolve);
    });
}

// aimateCircleX(0.2, 1, (translateX) => translateX > 50);
// aimateCircleScale(.2, 1, (scale) => scale > 1.5);
animationPromiseCircleScale(.5, 1, (scale) => scale > 1.5)
    .then(() => animationPromiseCircleX(0.5, 1, (translateX) => translateX > 50))
    .then(() => animationPromiseCircleScale(1.5, -1, (scale) => scale < 0.5))
    .then(() => animationPromiseCircleScale(.5, 1, (scale) => scale > 1.5)
    .then(() => animationPromiseCircleY(0.5, 1, (translatey) => translatey > 50))
    .then(() => animationPromiseCircleScale(1.5, -1, (scale) => scale < 0.5))
    .then(() => animationPromiseCircleScale(.5, 1, (scale) => scale > 1.5))
    .then(() => animationPromiseCircleX(50, -1, (translateX) => translateX < 0.5))
    .then(() => animationPromiseCircleScale(1.5, -1, (scale) => scale < 0.5))
    .then(() => animationPromiseCircleScale(.5, 1, (scale) => scale > 1.5))
    .then(() => animationPromiseCircleY(50, -1, (translatey) => translatey < 0.5))
    .then(() => animationPromiseCircleScale(1.5, -1, (scale) => scale < 0.5))
